...
所有.class可打包成jar文件，通过`hi_java_classpath`命令引入，然后通过`hi_java_servlet`调用需要的`servlet`提供服务。更新时，只需`reload`或者`restart`即可。实际上，`hi_java_servlet_cache_expires`配置了`servlet`过期时间，过期后，服务器会自动更新缓存的`servlet`。因此，不一定需要`reload`或者`restart`。
